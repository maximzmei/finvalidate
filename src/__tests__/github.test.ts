...
describe('postOrUpdateComment', () => {
  it('creates a new comment if no marker exists in any comment body', async () => {
    ...
    const url = await postOrUpdateComment(
      'token', 'org', 'repo', 1, 'review body', '<!-- finvalidate-review -->'
    );

    expect(mockCreateComment).toHaveBeenCalledWith({
      owner: 'org', repo: 'repo', issue_number: 1, body: 'review body',
    });
  ...
  it('updates existing comment when marker is found in its body', async () => {
    ...
    const url = await postOrUpdateComment(
      'token', 'org', 'repo', 1, 'updated review', '<!-- finvalidate-review -->'
    );

    expect(mockUpdateComment).toHaveBeenCalledWith({
      owner: 'org', repo: 'repo', comment_id: 42, body: 'updated review',
    });
  ...
  it('returns html_url from the created comment', async () => {
    ...
    mockListComments.mockResolvedValue({ data: [] });
    mockCreateComment.mockResolvedValue({ data: { html_url: 'https://expected-url.com' } });

    const url = await postOrUpdateComment('token', 'org', 'repo', 1, 'body', '<!-- marker -->');
