...
describe('formatDiff', () => {
  it('keeps lines starting with "+"', () => {
    ...
    const files = [{
      ...
      status: 'modified',
      patch: '+const amount = new Decimal(price);\n unchanged\n-const old = price;',
    }];
  ...
  it('skips files with status "removed"', () => {
    ...
    const files = [{ filename: 'src/old.ts', status: 'removed', patch: '+still indexed' }];
    const result = formatDiff(files, 10000);
  ...
  it('returns empty string if no additions exist in any file', () => {
    ...
    const files = [{ filename: 'src/x.ts', status: 'modified', patch: '-removed line' }];
    const result = formatDiff(files, 10000);
    expect(result.trim()).toBe('');
  });
});
...
describe('reviewPR', () => {
  const baseInput = {
    token: 'token', apiKey: 'key', model: 'model', maxDiffTokens: 6000,
  ...
  it('returns no violations and skips posting comment when Claude returns clean signal', async () => {
    ...
    vi.mocked(getPRFiles).mockResolvedValue([{
      filename: 'src/payment.ts', status: 'modified', patch: '+const x = new Decimal(1);',
    }] as any);
    vi.mocked(callClaude).mockResolvedValue('✅ No fintech rule violations detected.');
  ...
  it('posts comment, counts CRITICAL violations, and returns commentUrl', async () => {
    ...
    vi.mocked(getPRFiles).mockResolvedValue([{
      filename: 'src/payment.ts', status: 'modified', patch: '+const price = 1.1 + 2.2;',
    }] as any);
    vi.mocked(callClaude).mockResolvedValue(
      '🔴 CRITICAL: FIN-001 float arithmetic\n🔴 CRITICAL: FIN-003 toFixed'
    );
    vi.mocked(postOrUpdateComment).mockResolvedValue('https://github.com/org/repo/pull/1#issuecomment-99');

    const result = await reviewPR(baseInput);

    expect(result.violationsFound).toBe(true);
    expect(result.criticalCount).toBe(2);
    expect(result.commentUrl).toBe('https://github.com/org/repo/pull/1#issuecomment-99');
  });
});
